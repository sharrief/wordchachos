...
import { getWordList, getActiveSquareCoordinates, getUncheckedGuessIndex, getGuessesUsed, guessIsWrong, getGuessString, updateLetterStates } from "@game";
...
export function submitGuess(game: Game): Game {
  ...
  const {state, board: oldBoard, answer, guessesAllowed: gAllowed, guessLength: gLength } = game;
  ...
  if (state === GameState.active && letterCount === guessLength) {
    ...
    const { guessIndex, squareIndex } = getActiveSquareCoordinates(newBoard);
    game.guessIndex = guessIndex;
    game.squareIndex = squareIndex;
    if (noCorrectGuess) {
      if (guessesUsed === guessesAllowed) {
        /** This was the last guess. Game lost */
        return { ...game, board: newBoard, guessesChecked, state: GameState.loss }
      }
...
function allGuessesWrong(board: Board, guessLength: number): boolean {
  return board.reduce((allGuessesWrong, guess) => {
    if (!guessIsWrong(guess, guessLength)) return false;
    return allGuessesWrong;
  }, true as boolean);
}
