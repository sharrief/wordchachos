...
import { Board, Guess, KeyState, Square } from "@types";
...
export function updateLetterStates(guess: Guess, answer: string[]): Guess {
  ...
  const letterOccurrenceCounter = answer.reduce((accumulator, char) => {
    return { ...accumulator, [char]: (accumulator[char] ?? 0) + 1 };
  ...
  const squaresWithMatchCalculated = squaresWithPositionCalculated.map((square, characterIndex) => {
    const { letter } = square;
    let { state } = square;
